...
import { useAuth } from '/src/components/AuthContext.jsx'; // Adjust the import path accordingly
import { useNavigate } from 'react-router';

function Register() {
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const { user, login, logout, register, authError } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(username)
      ...
      if (!authError) {
          ...
          setUsername(''); // Reset state after submitting
          setPassword('');
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-100">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        {authError && (
            <p className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 w-full my-2">
